...
TEST (Punycode, ArabicEgyptian) {
  ...
  auto const encoded_arabic = "egbpdaj6bu4bxfgehfvwxn";
  std::string actual;
...
TEST (Punycode, ChineseSimplified) {
  ...
  auto const encoded_chinese_simplified = "ihqwcrb4cv8a8dqg056pqjye";
  std::string actual;
...
TEST (Punycode, ChineseTraditional) {
  ...
  auto const encoded = "ihqwctvzc91f659drss3x8bo0yb";
  std::string actual;
  uri::punycode::encode (orig, std::back_inserter (actual));
  EXPECT_EQ (uri::punycode::decode (encoded),
             uri::punycode::decode_result{orig});
...
TEST (Punycode, Czech) {
  ...
  auto const encoded = "Proprostnemluvesky-uyb24dma41a";
  std::string actual;
  uri::punycode::encode (orig, std::back_inserter (actual));
  EXPECT_EQ (actual, encoded);
  EXPECT_EQ (uri::punycode::decode (encoded),
...
TEST (Punycode, Hebrew) {
  ...
  auto const encoded = "4dbcagdahymbxekheh6e0a7fei0b";
  std::string actual;
  uri::punycode::encode (orig, std::back_inserter (actual));
  EXPECT_EQ (actual, encoded);
  EXPECT_EQ (uri::punycode::decode (encoded),
...
TEST (Punycode, HindiDevanagari) {
  ...
  auto const encoded = "i1baa7eci9glrd9b2ae1bj0hfcgg6iyaf8o0a1dig0cd";
  std::string actual;
  uri::punycode::encode (orig, std::back_inserter (actual));
  EXPECT_EQ (actual, encoded);
  EXPECT_EQ (uri::punycode::decode (encoded),
...
TEST (Punycode, JapaneseKanjiAndHiragana) {
  ...
  auto const encoded = "n8jok5ay5dzabd5bym9f0cm5685rrjetr6pdxa";
  std::string actual;
  uri::punycode::encode (orig, std::back_inserter (actual));
  EXPECT_EQ (actual, encoded);
  EXPECT_EQ (uri::punycode::decode (encoded),
...
TEST (Punycode, RussianCyrillic) {
  ...
  auto const encoded = "b1abfaaepdrnnbgefbadotcwatmq2g4l";
  std::string actual;
  uri::punycode::encode (orig, std::back_inserter (actual));
  EXPECT_EQ (actual, encoded);
  EXPECT_EQ (uri::punycode::decode (encoded),
...
TEST (Punycode, Spanish) {
  ...
  auto const encoded = "PorqunopuedensimplementehablarenEspaol-fmd56a";
  std::string actual;
  uri::punycode::encode (orig, std::back_inserter (actual));
  EXPECT_EQ (actual, encoded);
  EXPECT_EQ (uri::punycode::decode (encoded),
...
TEST (Punycode, Vietnamese) {
  ...
  auto const encoded = "TisaohkhngthchnitingVit-kjcr8268qyxafd2f1b9g";
  std::string actual;
  uri::punycode::encode (orig, std::back_inserter (actual));
  EXPECT_EQ (actual, encoded);
  EXPECT_EQ (uri::punycode::decode (encoded),
...
TEST (Punycode, ExampleL) {
  ...
  auto const encoded = "3B-ww4c5e180e575a65lsy2b";
  std::string actual;
  uri::punycode::encode (orig, std::back_inserter (actual));
  EXPECT_EQ (actual, encoded);
  EXPECT_EQ (uri::punycode::decode (encoded),
...
TEST (Punycode, ExampleM) {
  ...
  auto const encoded = "-with-SUPER-MONKEYS-pc58ag80a8qai00g7n9n";
  std::string actual;
  uri::punycode::encode (orig, std::back_inserter (actual));
  EXPECT_EQ (actual, encoded);
  EXPECT_EQ (uri::punycode::decode (encoded),
...
TEST (Punycode, ExampleN) {
  ...
  auto const encoded = "Hello-Another-Way--fc4qua05auwb3674vfr0b";
  std::string actual;
  uri::punycode::encode (orig, std::back_inserter (actual));
  EXPECT_EQ (actual, encoded);
  EXPECT_EQ (uri::punycode::decode (encoded),
...
TEST (Punycode, ExampleO) {
  ...
  auto const encoded = "2-u9tlzr9756bt3uc0v";
  std::string actual;
  uri::punycode::encode (orig, std::back_inserter (actual));
  EXPECT_EQ (actual, encoded);
  EXPECT_EQ (uri::punycode::decode (encoded),
...
TEST (Punycode, ExampleP) {
  ...
  auto const encoded = "MajiKoi5-783gue6qz075azm5e";
  std::string actual;
  uri::punycode::encode (orig, std::back_inserter (actual));
  EXPECT_EQ (actual, encoded);
  EXPECT_EQ (uri::punycode::decode (encoded),
...
TEST (Punycode, ExampleQ) {
  ...
  auto const encoded = "de-jg4avhby1noc0d";
  std::string actual;
  uri::punycode::encode (orig, std::back_inserter (actual));
  EXPECT_EQ (actual, encoded);
  EXPECT_EQ (uri::punycode::decode (encoded),
...
TEST (Punycode, ExampleR) {
  ...
  auto const encoded = "d9juau41awczczp";
  std::string actual;
  uri::punycode::encode (orig, std::back_inserter (actual));
  EXPECT_EQ (actual, encoded);
  EXPECT_EQ (uri::punycode::decode (encoded),
