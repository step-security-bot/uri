...
TEST (PctEncode, CodePointNeedsEncodeExhaustive) {
  ...
  // greater than U+007E (~).
  auto const is_c0 = [] (std::uint8_t c) { return c < 0x20 || c > 0x7E; };
  // The fragment percent-encode set is the C0 control percent-encode set and
  // U+0020 SPACE, U+0022 ("), U+003C (<), U+003E (>), and U+0060 (`).
  auto const is_fragment = [=] (std::uint8_t c) {
  ...
  auto const is_userinfo = [=] (std::uint8_t c) {
  ...
  auto const is_form_urlencoded = [=] (std::uint8_t c) noexcept {
  ...
  chars.resize (std::numeric_limits<std::uint8_t>::max ());
